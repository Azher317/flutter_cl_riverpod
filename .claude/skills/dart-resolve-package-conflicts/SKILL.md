...
> **This project — talker fork caveat.** [pubspec.yaml](../../../pubspec.yaml) pins
> `talker`, `talker_flutter`, and `talker_dio_logger` to a **patched git fork** via
> `dependency_overrides` (ref `v1.2-patched`). **All three overrides are required** — the
> flutter/dio packages depend on `talker: ^5.1.17` from pub.dev, so dropping any override
> silently pulls the unpatched core. The fork keeps version `5.1.17`, so `pub get` output
> and `flutter pub deps -s list` look identical either way. Before/after any resolution
> change, verify the fork is still in effect:
> `grep -A4 '^  talker:' pubspec.lock` (must show `url: https://github.com/Azher317/talker.git`),
> or `dart pub deps --json` (each of the three must report `"source": "git"`).
> Do **not** `--tighten` or upgrade these away, and never delete their override blocks to
> "resolve" a conflict. See [.claude/rules/pubspec.md](../../rules/pubspec.md).
