...
> **This project:** `flutter analyze` (or `dart analyze`) is only the analyzer pass. The
> Riverpod lints run through **`custom_lint`**, NOT the analyzer — you must also run
> `dart run custom_lint` (writes `custom_lint.log`, gitignored) to get the full picture.
> An analyzer-clean tree can still have Riverpod violations. Two more cautions:
> `*.g.dart` / `*.freezed.dart` are generated and analyzer-excluded — never hand-edit them
> or "fix" their output (CLAUDE.md rule 13); and prefer manual fixes over a blanket
> `dart fix --apply`, which can rewrite code in ways that fight the project's rules.
